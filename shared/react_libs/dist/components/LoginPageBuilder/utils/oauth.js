...
function sanitizeOAuthLog(data) {
    const sanitized = { ...data };
    // Never log client secrets, tokens, or codes
    if ('clientSecret' in sanitized)
 ...
 */
export function buildOAuth2Url(config) {
 ...
 */
export function buildCustomOAuth2Url(config) {
    const state = generateState();
    sessionStorage.setItem('oauth_state', state);
    const params = new URLSearchParams({
        client_id: config.clientId,
 ...
 */
export async function buildOIDCUrl(config) {
...
export function getProviderLabel(config) {
    ...
    if (config.provider in OAUTH2_PROVIDERS) {
        return OAUTH2_PROVIDERS[config.provider].label;
    }
 ...
 */
export function getProviderColors(provider) {
    const colors = {
        google: {
            background: 'bg-white',
            text: 'text-gray-700',
