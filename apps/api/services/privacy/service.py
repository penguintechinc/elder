...
class PrivacyService:
    """Synchronous PyDAL-backed DSAR operations."""

    @staticmethod
    def export_identity(db: Any, identity_id: int) -> dict[str, Any]:
        ...
        Raises:
            LookupError: If no identity with this id exists.
        """
    ...
    @staticmethod
    def erase_identity(
        db: Any, identity_id: int, tenant_legal_hold: bool
    ) -> dict[str, Any]:
        ...
        Args:
            ...
            tenant_legal_hold: True if the owning tenant has an active legal
                hold -- blocks erasure rather than purging preservable data.
        ...
        db(db.identities.id == identity_id).update(
        ...
        db.commit()
        logger.info("dsar_identity_anonymized", identity_id=identity_id)
        return {"identity_id": identity_id, "anonymized": True}

    @staticmethod
    def set_do_not_sell(db: Any, identity_id: int, opted_out: bool) -> dict[str, Any]:
        ...
        db.commit()
        logger.info(
            "dsar_consent_updated", identity_id=identity_id, opted_out=opted_out
        )
        return {"identity_id": identity_id, "do_not_sell_share": opted_out}

    @staticmethod
    def bulk_erase(
        db: Any, identity_ids: list[int], tenant_legal_hold: bool
    ) -> dict[str, Any]:
        """Enterprise admin bulk erasure -- same anonymization, many identities at once.

        Never raises on a per-identity failure -- collects per-id outcomes so
        one bad id in a bulk batch doesn't abort the rest.

        Args:
            db: penguin-dal DAL instance.
            identity_ids: identities.id values to erase.
            tenant_legal_hold: True if the owning tenant has an active legal
                hold -- blocks every identity in the batch.
        ...
        for iid in identity_ids:
            try:
                results.append(
                    PrivacyService.erase_identity(db, iid, tenant_legal_hold)
                )
