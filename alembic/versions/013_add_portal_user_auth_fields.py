...
def upgrade():
    ...
    if "password_hash" not in columns:
        op.add_column(
            "portal_users", sa.Column("password_hash", sa.String(255), nullable=True)
        )

    if "global_role" not in columns:
        op.add_column(
            "portal_users", sa.Column("global_role", sa.String(50), nullable=True)
        )

    # Copy password to password_hash for backward compatibility
    op.execute(
        "UPDATE portal_users SET password_hash = password WHERE password_hash IS NULL"
    )
