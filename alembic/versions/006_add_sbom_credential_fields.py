...
def upgrade():
    ...
    if "credential_type" not in existing:
        op.add_column(
            "sbom_scans", sa.Column("credential_type", sa.String(50), nullable=True)
        )
    if "credential_id" not in existing:
        op.add_column(
            "sbom_scans", sa.Column("credential_id", sa.Integer(), nullable=True)
        )
    if "credential_mapping" not in existing:
        op.add_column(
            "sbom_scans", sa.Column("credential_mapping", sa.JSON(), nullable=True)
        )
    ...
    if "credential_mapping" not in existing:
        op.add_column(
            "sbom_scan_schedules",
            sa.Column("credential_mapping", sa.JSON(), nullable=True),
        )
