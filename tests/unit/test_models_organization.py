...
class TestOrganizationModel:
    ...
    def test_organization_display_name(self, app):
        """Test display name assignment."""
        db = app.db
        org_id = db.organizations.insert(
            name="internal-name",
        ...
        org = db.organizations[org_id]
        assert org.display_name == "Engineering Team"

        # Cleanup
        db(db.organizations.id == org_id).delete()
        db.commit()

    def test_organization_cloud_provider(self, app):
        """Test cloud provider assignment."""
        db = app.db
        org_id = db.organizations.insert(
            name="cloud-org",
            cloud_provider="aws",
        ...
        org = db.organizations[org_id]
        assert org.cloud_provider == "aws"
        assert org.cloud_account_id == "123456789012"
    ...
    def test_organization_village_id(self, app):
        ...
        assert org.village_id is None

        # Now test setting village_id explicitly (AD-3 format: TTTTTTTT-OOOOOOOOOOOOOOOO)
        valid_village_id = "00000001-0000000000000002"
        db(db.organizations.id == org_id).update(village_id=valid_village_id)
        db.commit()

        org = db.organizations[org_id]
        assert org.village_id == valid_village_id
    ...
    def test_organization_update(self, app):
        ...
        db(db.organizations.id == org_id).update(
            name="Updated Name", display_name="New Display"
        )
        db.commit()

        org = db.organizations[org_id]
        assert org.name == "Updated Name"
        assert org.display_name == "New Display"
